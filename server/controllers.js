...
const addColaborador = async (req, res) => {
  try {
      ...
      try {
          ...
          if (!isAdmin) {
              return res.status(403).json({ message: 'Apenas administradores podem adicionar colaboradores' });
          }
      } catch (error) {
...
const deleteColaborador = async (req, res) => {
  try {
      ...
      try {
          ...
          // Apenas admin pode excluir colaboradores
          if (!isAdmin) {
              return res.status(403).json({ message: 'Apenas administradores podem excluir colaboradores' });
          }
      } catch (error) {
          return res.status(401).json({ message: 'Token inválido' });
      }
